...
export function Footer() {
  const [content, setContent] = useState<any>(defaultFooter)
  ...
  const fetchContent = async () => {
    try {
      const res = await fetch('/api/admin/landing-page?section=footer')
      if (res.ok) {
  ...
  if (loading) {
    return <footer className="bg-ink text-cream py-20 animate-pulse" />
  }
            ...
            <Reveal delay={100}>
              <div>
                ...
                <ul className="space-y-4">
                  {shopLinks.map((link) => (
                    <li key={link.label}>
                      <Link href={link.href} className="text-[15px] text-cream/60 hover:text-terracotta-light transition-colors duration-300 font-light">{link.label}</Link>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <Reveal delay={250}>
              <div>
                ...
                <div className="flex gap-3 mt-8">
                  <Link href={content.instagram_url} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-cream/5 flex items-center justify-center hover:bg-terracotta/20 hover:text-terracotta-light transition-all duration-300" aria-label="Instagram">
                    <Instagram className="w-[18px] h-[18px]" strokeWidth={1.5} />
                  </Link>
                  <Link href={content.facebook_url} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-cream/5 flex items-center justify-center hover:bg-terracotta/20 hover:text-terracotta-light transition-all duration-300" aria-label="Facebook">
                    <Facebook className="w-[18px] h-[18px]" strokeWidth={1.5} />
                  </Link>
                  <Link href={content.twitter_url} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-cream/5 flex items-center justify-center hover:bg-terracotta/20 hover:text-terracotta-light transition-all duration-300" aria-label="Twitter">
              ...
              <div className="flex items-center gap-6">
                <div className="flex items-center gap-3 text-[13px] text-cream/40">
                  ...
                  {['Visa', 'Mastercard', 'UPI', 'Paytm'].map((method) => (
                    <span key={method} className="font-light">{method}</span>
                  ))}
                </div>
              </div>
            </div>
